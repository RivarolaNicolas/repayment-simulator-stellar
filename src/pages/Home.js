...
const Home = (props) => {
	...
	useEffect(() => {
		setTwoPercentOfInterestPaid(calculate2PercentOfInterestPaid(loanAmount, repaymentAmount, totalLoanAmount));
	}, [loanAmount, repaymentAmount, totalLoanAmount, twoPercentOfInterestPaid]);

	useEffect(() => {
		setBorrowerRewards(calculateBorrowerRewards(minimumRepayment, repaymentAmount, twoPercentOfInterestPaid));
	...
	function handleSubmitTransactionClicked(e) {
		e.preventDefault();
		transactionSubmitter(borrowerAccountPrivateKey, borrowerRewards, twoPercentOfInterestPaid, setTransactionSuccessful);
	}
		...
		<div className='overflow-x-hidden overflow-y-hidden '>
			<div className='bg-white rounded-lg shadow sm:max-w-md sm:w-full sm:mx-auto sm:overflow-hidden md:my-10 '>
				<div className='px-4 py-8 sm:px-10'>
					...
					<div className='mt-6'>
						<div className='w-full space-y-6'>
							...
							<div className='w-full'>
								<div className=' relative '>
									<label forhtml='private-key' className='relative text-gray-400 focus-within:text-gray-600 block'>
									...
									{borrowerAccountPrivateKey == false ? (
										<div className='text-gray-500 mt-2'>Enter valid Private Key to be able to submit transaction</div>
									) : (
										''
									)}
								</div>
				...
				<div className='px-4 py-6 border-t-2 border-gray-200 bg-gray-50 sm:px-10'>
					{<TransactionAlert transactionSuccessful={transactionSuccessful} hidden={hide} alertMessage={alertMessage} />}
				</div>
			</div>
		</div>
	);
